...
class Mechanism(ABC):
    ...
    def log_end(self):
        if self.verbose:
            print(f'===  RESULTS  ===')
            print(f'Accepted: {print_dict(self.allocation.matched)}')
...
example_cermat_applications = {
    example_cermat_student_names[k]: tuple((example_cermat_school_names[sch]
        for sch in v)) for k, v in example_cermat_applications.items()}

example_cermat_exams = {
    example_cermat_school_names[k]: tuple((example_cermat_student_names[st]
        for st in v)) for k, v in example_cermat_exams.items()}
